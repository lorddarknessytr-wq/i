...
ساخت هر تصویر حدود ۱ تا ۳ دقیقه طول می‌کشد.`;
...
async function handle(platform, chatId, text, env) {
  ...
  return reply(ok ? "⏳ در حال ساخت تصویر... (حدود ۱ تا ۳ دقیقه)" : "❌ ارتباط با سرور ساخت تصویر برقرار نشد.");
}
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === "/telegram") {
      ...
      if (msg && msg.text) ctx.waitUntil(handle("telegram", msg.chat.id, msg.text, env));
